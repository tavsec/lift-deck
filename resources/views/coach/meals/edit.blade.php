<x-layouts.coach>
    ...
    <div class="space-y-6">
        <!-- Header -->
        <div>
            <a href="{{ route('coach.meals.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-4">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
                Back to Meal Library
            </a>
            <h1 class="text-2xl font-bold text-gray-900">Edit Meal</h1>
            <p class="mt-1 text-sm text-gray-500">Update meal information.</p>
        </div>
            ...
            <form method="POST" action="{{ route('coach.meals.update', $meal) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $meal->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="calories" class="block text-sm font-medium text-gray-700">Calories <span class="text-red-500">*</span></label>
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
                    <a href="{{ route('coach.meals.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-medium text-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        Cancel
                    </a>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="mt-1 text-sm text-gray-500">Archiving this meal will remove it from the library. Existing meal logs will not be affected.</p>
            <div class="mt-4">
