<x-layouts.coach>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="font-display text-2xl font-semibold text-[#222222] dark:text-gray-100">{{ __('coach.meals.index.heading') }}</h1>
                <p class="mt-1 text-sm text-[#8e8e93] dark:text-gray-500">{{ __('coach.meals.index.subtitle') }}</p>
            </div>
            <a href="{{ route('coach.meals.create') }}" class="inline-flex items-center px-4 py-2 bg-[#181e25] dark:bg-gray-700 text-white text-sm font-semibold rounded-lg hover:bg-gray-800 transition-colors">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-card p-4">
            <form method="GET" action="{{ route('coach.meals.index') }}" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('coach.meals.index.search_placeholder') }}" class="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-[#222222] dark:text-gray-100 placeholder-[#8e8e93] dark:placeholder-gray-500 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#1456f0] focus:ring-2 focus:ring-[#1456f0]/20 transition-colors duration-150">
                </div>
        ...
        @if($meals->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($meals as $meal)
                    <a href="{{ route('coach.meals.edit', $meal) }}" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-card overflow-hidden hover:border-gray-300 dark:hover:border-gray-700 transition-colors">
                        <div class="p-5">
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-semibold text-[#222222] dark:text-gray-100 truncate">{{ $meal->name }}</h3>
                                </div>
                            </div>
                            <div class="mt-3 flex items-baseline gap-1">
                                <span class="text-2xl font-semibold text-[#222222] dark:text-gray-100">{{ $meal->calories }}</span>
                            ...
                            @if($meal->description)
                                <p class="mt-2 text-sm text-[#8e8e93] dark:text-gray-400 line-clamp-2">{{ $meal->description }}</p>
                            @endif
                        </div>
                    </a>
                @endforeach
        ...
        @else
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-card">
                <div class="text-center py-12">
                    ...
                    <h3 class="text-sm font-semibold text-[#222222] dark:text-gray-100">{{ __('coach.meals.index.no_meals') }}</h3>
