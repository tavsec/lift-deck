                ...
                @if($conversations->count() > 0)
                    <div class="bg-white rounded-lg shadow divide-y divide-gray-200">
                        @foreach($conversations as $conversation)
                            <a href="{{ route('coach.messages.show', $conversation['client']) }}" class="block hover:bg-gray-50 transition-colors">
                ...
                @else
                    <div class="bg-white rounded-lg shadow">
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Start a conversation with one of your clients.</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="space-y-4">
                ...
                @if($clientsWithoutMessages->count() > 0)
                    <div class="bg-white rounded-lg shadow divide-y divide-gray-200">
                        @foreach($clientsWithoutMessages as $client)
                            <a href="{{ route('coach.messages.show', $client) }}" class="block px-4 py-3 hover:bg-gray-50 transition-colors">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center">
                                        <span class="text-sm font-medium text-gray-700">{{ strtoupper(substr($client->name, 0, 1)) }}</span>
                                    </div>
                                    <div class="ml-3">
                ...
                @else
                    <div class="bg-white rounded-lg shadow p-4">
                        <p class="text-sm text-gray-500 text-center">All clients have active conversations.</p>
                    </div>
                @endif
            </div>
        </div>
