                        ...
                        @foreach($macroGoals as $goal)
                            <div class="flex items-start justify-between p-3 rounded-md {{ $currentGoal && $goal->id === $currentGoal->id ? 'bg-blue-50 dark:bg-blue-900/30' : 'bg-gray-50 dark:bg-gray-900' }}">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ $goal->effective_date->format('M d, Y') }}</p>
                                    @if($goal->notes)
                                        <p class="text-xs text-gray-500 dark:text-gray-400 italic mt-1">{{ $goal->notes }}</p>
                                    @endif
                                </div>
                                <form method="POST" action="{{ route('coach.macro-goals.destroy', $goal) }}" onsubmit="return confirm('Remove this goal?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-gray-400 dark:text-gray-500 hover:text-red-500">
