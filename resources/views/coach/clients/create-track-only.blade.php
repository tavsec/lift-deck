<x-layouts.coach>
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('coach.clients.create_track_only.heading') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('coach.clients.create_track_only.subtitle') }}</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-6">
            <form method="POST" action="{{ route('coach.clients.store-track-only') }}" class="space-y-6">
                ...
                <!-- Name -->
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.clients.create_track_only.name') }} <span class="text-red-500">*</span></label>
                    <div class="mt-1">
                ...
                <!-- Phone -->
                <div>
                    <label for="phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.clients.create_track_only.phone') }} <span class="text-gray-400 font-normal">{{ __('coach.clients.create_track_only.optional') }}</span></label>
                    <div class="mt-1">
                ...
                <div class="flex items-center gap-4 pt-2">
                    ...
                    <a href="{{ route('coach.clients.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ __('coach.clients.create_track_only.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
