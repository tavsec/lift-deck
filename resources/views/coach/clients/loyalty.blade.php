<x-layouts.coach>
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900">{{ __('coach.clients.loyalty.heading', ['name' => $client->name]) }}</h1>
        </div>
        ...
        <!-- XP Earned -->
        <x-bladewind::card>
            <h2 class="text-base font-semibold text-gray-900 mb-4">{{ __('coach.clients.loyalty.xp_earned') }}</h2>
            ...
            @if($xpTransactions->isEmpty())
                <div class="py-8 text-center text-gray-400">
                    ...
                    <p class="text-sm">{{ __('coach.clients.loyalty.no_xp') }}</p>
                </div>
            @else
                <ul class="divide-y divide-gray-100">
                    @foreach($xpTransactions as $transaction)
                        <li class="py-3 flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-800">{{ $transaction->xpEventType?->name ?? __('coach.clients.loyalty.xp_event') }}</p>
        ...
        <!-- Points Spent -->
        <x-bladewind::card>
            <h2 class="text-base font-semibold text-gray-900 mb-4">{{ __('coach.clients.loyalty.points_spent') }}</h2>
            ...
            @if($redemptions->isEmpty())
                <div class="py-8 text-center text-gray-400">
                    ...
                    <p class="text-sm">{{ __('coach.clients.loyalty.no_redemptions') }}</p>
                </div>
            @else
                <ul class="divide-y divide-gray-100">
                    @foreach($redemptions as $redemption)
                        ...
                        <li class="py-3 flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-800">{{ $redemption->reward?->name ?? __('coach.clients.loyalty.deleted_reward') }}</p>
