<x-layouts.coach>
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('coach.exercises.edit.heading') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('coach.exercises.edit.subtitle') }}</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-6">
            <form method="POST" action="{{ route('coach.exercises.update', $exercise) }}" class="space-y-6">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.exercises.edit.name') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="muscle_group" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.exercises.edit.muscle_group') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.exercises.edit.description') }}</label>
                    <textarea name="description" id="description" rows="4"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $exercise->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="video_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('coach.exercises.edit.video_url') }}</label>
                    ...
                    @error('video_url')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('coach.exercises.edit.video_hint') }}</p>
                </div>

                <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                    <a href="{{ route('coach.exercises.show', $exercise) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md font-medium text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
