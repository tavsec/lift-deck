<x-layouts.coach>
    <x-slot:title>{{ __('coach.day_plans.create.heading') }}</x-slot:title>
    ...
    <div class="space-y-6">
        ...
        <div>
            <a href="{{ route('coach.day-plans.index') }}" class="inline-flex items-center text-sm text-[#8e8e93] dark:text-gray-400 hover:text-[#45515e] dark:hover:text-gray-100 mb-4">
                ...
                {{ __('coach.day_plans.create.back') }}
            </a>
            <h1 class="font-display text-2xl font-semibold text-[#222222] dark:text-gray-100">{{ __('coach.day_plans.create.heading') }}</h1>
            <p class="mt-1 text-sm text-[#8e8e93] dark:text-gray-500">{{ __('coach.day_plans.create.subtitle') }}</p>
        </div>

        @include('coach.day-plans._form', [
            'action' => route('coach.day-plans.store'),
            'method' => 'POST',
