    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                ...
                <div class="flex items-center gap-3">
                    <h1 class="font-display text-[30px] font-bold text-[#181b22] dark:text-[#f0f2f5] tracking-tight">{{ $program->name }}</h1>
            ...
            <div class="flex gap-2">
                <a href="{{ route('coach.programs.assign', $program) }}" class="inline-flex items-center px-4 py-2.5 bg-[#181b22] dark:bg-[#c6f24e] border border-transparent rounded-lg font-semibold text-sm text-white dark:text-[#14180a] hover:bg-[#2d3748] dark:hover:bg-[#b4e438] focus:outline-none transition ease-in-out duration-150">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/>
                    </svg>
                    {{ __('coach.programs.show.assign') }}
                </a>
                <a href="{{ route('coach.programs.edit', $program) }}" class="inline-flex items-center px-4 py-2.5 border border-[rgba(18,22,31,0.14)] dark:border-[rgba(255,255,255,0.12)] rounded-lg font-medium text-sm text-[#555b66] dark:text-[#a4abb6] bg-white dark:bg-[#11141a] hover:bg-[#f3f5f7] dark:hover:bg-[#1d2027] focus:outline-none transition ease-in-out duration-150">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <form method="POST" action="{{ route('coach.programs.destroy', $program) }}" onsubmit="return confirm('{{ __('coach.programs.show.delete_confirm') }}');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="inline-flex items-center px-4 py-2.5 border border-red-200 dark:border-red-800 rounded-lg font-medium text-sm text-red-600 dark:text-red-400 bg-white dark:bg-[#11141a] hover:bg-red-50 dark:hover:bg-red-900/20 focus:outline-none transition ease-in-out duration-150">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        @if($program->description)
            <div class="bg-white dark:bg-[#16191f] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.04),0_4px_16px_rgba(18,22,31,.045)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_20px_rgba(0,0,0,.3)] p-6">
                <h2 class="text-xs font-medium text-[#8c93a0] dark:text-[#6b7280] uppercase tracking-wide mb-2">{{ __('coach.programs.show.description') }}</h2>
                <p class="text-sm text-[#555b66] dark:text-[#a4abb6]">{{ $program->description }}</p>
            </div>
        @endif
        ...
        @if($assignedClients->count() > 0)
            <div class="bg-white dark:bg-[#16191f] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.04),0_4px_16px_rgba(18,22,31,.045)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_20px_rgba(0,0,0,.3)] p-6">
                <h2 class="font-display text-base font-semibold text-[#181b22] dark:text-[#f0f2f5] mb-4">{{ __('coach.programs.show.assigned_clients', ['n' => $assignedClients->count()]) }}</h2>
                <div class="flex flex-wrap gap-3">
                    @foreach($assignedClients as $assignment)
                        <div class="inline-flex items-center gap-2">
                            <a href="{{ route('coach.clients.show', $assignment->client) }}" class="inline-flex items-center px-3 py-2 rounded-lg bg-[#f3f5f7] dark:bg-[#1d2027] hover:bg-gray-100 dark:hover:bg-[#2a2f3a] transition-colors">
                                <div class="h-8 w-8 rounded-full flex items-center justify-center overflow-hidden mr-2 bg-gradient-to-br from-[#7c5cff] to-[#c6f24e]">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-[#181b22] dark:text-[#f0f2f5]">{{ $assignment->client->name }}</p>
                                    <p class="text-xs text-[#8c93a0] dark:text-[#6b7280]">
                                        ...
                                        @else
                                            <span class="text-[#8c93a0] dark:text-[#6b7280]">{{ __('coach.programs.show.completed') }}</span>
                                        @endif
                                        - {{ __('coach.programs.show.started') }} {{ $assignment->started_at->format('M d, Y') }}
                                    </p>
                                </div>
                            </a>
                            <a href="{{ route('coach.programs.assignments.targets.edit', [$program, $assignment]) }}" class="inline-flex items-center px-2.5 py-1.5 border border-[rgba(18,22,31,0.14)] dark:border-[rgba(255,255,255,0.12)] rounded-lg text-xs font-medium text-[#555b66] dark:text-[#a4abb6] bg-white dark:bg-[#11141a] hover:bg-[#f3f5f7] dark:hover:bg-[#1d2027] transition-colors">
        ...
        <!-- Workouts -->
        <div class="space-y-4">
            <h2 class="font-display text-base font-semibold text-[#181b22] dark:text-[#f0f2f5]">{{ __('coach.programs.show.workouts') }}</h2>
            ...
            @if($program->workouts->count() > 0)
                @foreach($program->workouts as $workout)
                    <div class="bg-white dark:bg-[#16191f] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.04),0_4px_16px_rgba(18,22,31,.045)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_20px_rgba(0,0,0,.3)] overflow-hidden">
                        <div class="px-6 py-4 bg-[#f3f5f7] dark:bg-[#1d2027] border-b border-[rgba(18,22,31,0.06)] dark:border-[rgba(255,255,255,0.06)]">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-[#8c93a0] dark:text-[#6b7280]">Day {{ $workout->day_number }} &middot; {{ $workout->exercises->count() }} exercises</p>
                                </div>
                            </div>
                            @if($workout->notes)
                        ...
                        @else
                            <div class="px-6 py-8 text-center text-sm text-[#8c93a0] dark:text-[#6b7280]">
            ...
            @else
                <div class="bg-white dark:bg-[#16191f] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.04),0_4px_16px_rgba(18,22,31,.045)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_20px_rgba(0,0,0,.3)]">
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-sm font-semibold text-[#181b22] dark:text-[#f0f2f5]">{{ __('coach.programs.show.no_workouts') }}</h3>
                        <p class="mt-1 text-sm text-[#8c93a0] dark:text-[#6b7280]">{{ __('coach.programs.show.no_workouts_description') }}</p>
                        <div class="mt-6">
                            <a href="{{ route('coach.programs.edit', $program) }}" class="inline-flex items-center px-4 py-2.5 bg-[#181b22] dark:bg-[#c6f24e] border border-transparent rounded-lg font-semibold text-sm text-white dark:text-[#14180a] hover:bg-[#2d3748] dark:hover:bg-[#b4e438] transition ease-in-out duration-150">
