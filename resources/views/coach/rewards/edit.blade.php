<x-layouts.coach>
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <h1 class="font-display text-2xl font-semibold text-[#222222] dark:text-gray-100">{{ __('coach.rewards.edit.heading') }}</h1>
            <p class="text-sm text-[#8e8e93] dark:text-gray-500 mt-0.5">{{ __('coach.rewards.edit.subtitle') }}</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-card p-6">
            <form method="POST" action="{{ route('coach.rewards.update', $reward) }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-[#45515e] dark:text-gray-300 mb-1.5">{{ __('coach.rewards.edit.name') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="description" class="block text-sm font-medium text-[#45515e] dark:text-gray-300 mb-1.5">{{ __('coach.rewards.edit.description') }}</label>
                    <textarea name="description" id="description" rows="3"
                        class="w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-[#222222] dark:text-gray-100 placeholder-[#8e8e93] dark:placeholder-gray-500 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#1456f0] focus:ring-2 focus:ring-[#1456f0]/20 transition-colors duration-150 @error('description') border-red-300 dark:border-red-600 @enderror">{{ old('description', $reward->description) }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="points_cost" class="block text-sm font-medium text-[#45515e] dark:text-gray-300 mb-1.5">{{ __('coach.rewards.edit.points_cost') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="stock" class="block text-sm font-medium text-[#45515e] dark:text-gray-300 mb-1.5">{{ __('coach.rewards.edit.stock') }}</label>
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-[#45515e] dark:text-gray-300 mb-1.5">{{ __('coach.rewards.edit.image') }}</label>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-card p-6">
            ...
            <p class="mt-1 text-sm text-[#8e8e93] dark:text-gray-500">{{ __('coach.rewards.edit.archive_description') }}</p>
            <div class="mt-4">
