    ...
    <div class="px-4 py-5 space-y-4" x-data="checkIn()">
        ...
        <div class="mb-5">
            <div class="flex items-center justify-between">
                <h1 class="font-display text-2xl font-bold text-[#181b22] dark:text-[#f0f2f5] tracking-tight">{{ __('client.check_in.heading') }}</h1>
                <a href="{{ route('client.check-in.history') }}" class="text-sm font-semibold text-[#5c7a10] dark:text-[#c6f24e] hover:opacity-80">
            ...
            <div class="mt-3 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <input type="date" x-model="currentDate" @change="navigateToDate()" max="{{ now()->format('Y-m-d') }}"
                        class="rounded-lg border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-[#c6f24e] focus:ring-[#c6f24e] text-sm text-[#181b22] dark:text-gray-100">
            ...
            <form method="POST" action="{{ route('client.check-in.store') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                @foreach($assignedMetrics as $metric)
                    <div class="bg-white dark:bg-[#181b21] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.05),0_5px_16px_rgba(18,22,31,.05)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_18px_rgba(0,0,0,.3)] p-5">
                        <label class="block text-sm font-semibold text-[#181b22] dark:text-[#f0f2f5] mb-1">
                            {{ $metric->name }}
                            @if($metric->unit)
                        ...
                        @if($metric->description)
                            <p class="text-xs text-[#8c93a0] dark:text-[#6b7280] mb-3">{{ $metric->description }}</p>
                        @endif

                        @if($metric->type === 'number')
                            <input type="number" step="any" name="metrics[{{ $metric->id }}]"
                                value="{{ $existingLogs->get($metric->id)?->value }}"
                                class="block w-full rounded-lg border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-[#c6f24e] focus:ring-[#c6f24e] text-sm text-[#181b22]"
                                ...
                                <div class="flex items-center justify-between gap-1">
                                    @for($i = $metric->scale_min; $i <= $metric->scale_max; $i++)
                                        <button type="button" @click="value = value === '{{ $i }}' ? '' : '{{ $i }}'"
                                            :class="value === '{{ $i }}' ? 'bg-[#c6f24e] text-[#14180a] border-[#c6f24e]' : 'bg-white dark:bg-gray-800 text-[#555b66] dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'"
                        ...
                        @elseif($metric->type === 'boolean')
                            ...
                            <div x-data="{ value: '{{ $currentVal ?? '' }}' }" class="flex gap-3">
                                ...
                                <button type="button" @click="value = value === '1' ? '' : '1'"
                                    :class="value === '1' ? 'bg-green-600 text-white border-green-600' : 'bg-white dark:bg-gray-800 text-[#555b66] dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                    class="flex-1 py-2 text-sm font-medium border rounded-lg transition-colors">
                                    {{ __('client.check_in.yes') }}
                                </button>
                                <button type="button" @click="value = value === '0' ? '' : '0'"
                                    :class="value === '0' ? 'bg-red-500 text-white border-red-500' : 'bg-white dark:bg-gray-800 text-[#555b66] dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                    class="flex-1 py-2 text-sm font-medium border rounded-lg transition-colors">
                        ...
                        @elseif($metric->type === 'text')
                            <textarea name="metrics[{{ $metric->id }}]" rows="2"
                                class="block w-full rounded-lg border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-[#c6f24e] focus:ring-[#c6f24e] text-sm"
                        ...
                        @elseif($metric->type === 'image')
                            ...
                            <div x-data="imageUpload({{ $metric->id }}, {{ $hasMedia ? 'true' : 'false' }}, '{{ $thumbUrl }}')" class="space-y-2">
                                ...
                                <template x-if="!previewUrl && (!hasExisting || removed)">
                                    <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-200 dark:border-gray-700 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <svg class="w-8 h-8 mb-2 text-[#8c93a0] dark:text-[#6b7280]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            </svg>
                                            <p class="text-xs text-[#8c93a0] dark:text-[#6b7280]">{{ __('client.check_in.tap_to_upload') }}</p>
                                        </div>
                                ...
                                <template x-if="converting">
                                    <p class="text-xs text-[#5c7a10] dark:text-[#c6f24e]">Converting image...</p>
                                </template>
                            </div>
                        @endif
                    </div>
                ...
                <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-3 bg-[#c6f24e] text-[#14180a] text-sm font-semibold rounded-xl hover:bg-[#b4e438] transition-colors">
        ...
        @else
            <div class="bg-white dark:bg-[#181b21] rounded-xl border border-[rgba(18,22,31,0.09)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_1px_2px_rgba(18,22,31,.05),0_5px_16px_rgba(18,22,31,.05)] dark:shadow-[0_1px_2px_rgba(0,0,0,.4),0_6px_18px_rgba(0,0,0,.3)] p-5">
                <div class="text-center py-8">
                    ...
                    <h3 class="mt-3 text-sm font-semibold text-[#181b22] dark:text-[#f0f2f5]">{{ __('client.check_in.no_metrics') }}</h3>
                    <p class="mt-1 text-sm text-[#8c93a0] dark:text-[#6b7280]">{{ __('client.check_in.no_metrics_description') }}</p>
                </div>
            </div>
        @endif
    </div>
