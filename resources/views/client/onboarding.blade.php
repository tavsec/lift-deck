<x-guest-layout>
    ...
    <form method="POST" action="{{ route('client.onboarding.store') }}">
                ...
                @if($field->type === 'select')
                    <div class="mt-2 space-y-2">
                        @foreach($field->options ?? [] as $option)
                            <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('fields.' . $field->id) === $option ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
        ...
        @empty
            <p class="text-sm text-gray-500 mb-6">No onboarding questions have been set up yet.</p>
        @endforelse
