<x-layouts.client>
    ...
    <div class="py-6 space-y-6">
        <!-- Welcome Greeting -->
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ __('client.dashboard.hey', ['name' => auth()->user()->name]) }}</h1>
            @if ($coach)
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('client.dashboard.your_coach', ['coach_name' => $coach->name]) }}</p>
            @endif
        </div>
        ...
        <x-bladewind::card>
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('client.dashboard.active_program') }}</h2>
                ...
                @if($activeProgram)
                    <div class="space-y-2">
                        ...
                        <div class="pt-2">
                            <a href="{{ route('client.program') }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">{{ __('client.dashboard.view_full_program') }}</a>
                        </div>
                    </div>
                @else
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <x-bladewind::card>
                <div class="space-y-2">
                    <h3 class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('client.dashboard.this_week') }}</h3>
                    <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $weeklyWorkoutCount }} / {{ $weeklyWorkoutTarget }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('client.dashboard.workouts_completed') }}</p>
                </div>
            </x-bladewind::card>
            ...
            <x-bladewind::card>
                <div class="space-y-2">
                    <h3 class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('client.dashboard.last_workout') }}</h3>
                    @if($lastWorkout)
                    ...
                    @else
                        <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('client.dashboard.no_workouts') }}</p>
                    @endif
                </div>
            </x-bladewind::card>
        </div>
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <span class="text-sm font-medium text-gray-700">{{ number_format($xpSummary?->total_xp ?? 0) }} {{ __('client.dashboard.xp') }}</span>
                    </div>
                    <span class="text-sm font-semibold text-blue-600">{{ number_format($xpSummary?->available_points ?? 0) }} {{ __('client.dashboard.pts') }}</span>
                </div>
                ...
                <div>
                    ...
                    @if($nextLevel)
                        <p class="text-xs text-gray-400 mt-1">{{ number_format($nextLevelXp - $currentXp) }} {{ __('client.dashboard.xp_to_go') }}</p>
                    @endif
                </div>

                @if($recentAchievements->isNotEmpty())
                    <div class="flex items-center gap-2 pt-1">
                        <span class="text-xs text-gray-500">{{ __('client.dashboard.recent') }}</span>
                ...
                <div class="flex items-center gap-4 pt-2 border-t border-gray-100">
                    <a href="{{ route('client.rewards') }}" class="text-sm text-blue-600 hover:text-blue-800">{{ __('client.dashboard.rewards_shop') }}</a>
                    <a href="{{ route('client.achievements') }}" class="text-sm text-blue-600 hover:text-blue-800">{{ __('client.dashboard.achievements') }}</a>
                    <a href="{{ route('client.loyalty') }}" class="text-sm text-blue-600 hover:text-blue-800">{{ __('client.dashboard.points_history') }}</a>
                </div>
            </div>
        </x-bladewind::card>
        @endif
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('client.dashboard.check_in_progress', ['n' => $todayLogCount, 'total' => $assignedMetricCount]) }}</p>
                            </div>
                        </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('client.dashboard.your_coach_section') }}</p>
                        </div>
                    </div>
                    <x-bladewind::button
                        tag="a"
