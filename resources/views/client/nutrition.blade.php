                ...
                @foreach($macros as $macro)
                    ...
                    <div>
                        <div class="flex justify-between text-sm mb-1">
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $macro['label'] }}</span>
                            <span class="text-gray-500 dark:text-gray-400">{{ number_format($macro['current'], $macro['unit'] === 'kcal' ? 0 : 1) }} / {{ number_format($macro['target'], $macro['unit'] === 'kcal' ? 0 : 1) }}{{ $macro['unit'] }}</span>
                        </div>
                ...
                @foreach($mealLogs->groupBy('meal_type') as $type => $logs)
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                        <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $type }}</h3>
                        </div>
                        <div class="divide-y divide-gray-50 dark:divide-gray-700">
                            @foreach($logs as $log)
                                <div class="px-4 py-3 flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $log->name }}</p>
                ...
                <div x-show="mode === 'library'" class="space-y-4">
                    <div>
                        <input type="text" x-model="mealSearch" @input.debounce.300ms="searchMeals()" placeholder="Search meals..."
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
                    ...
                    <div x-show="libraryMeals.length > 0" class="max-h-60 overflow-y-auto space-y-2">
                        <template x-for="meal in libraryMeals" :key="meal.id">
                            <button @click="selectLibraryMeal(meal)" type="button"
                                :class="selectedMeal && selectedMeal.id === meal.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                class="w-full text-left p-3 rounded-md border transition-colors">
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100" x-text="meal.name"></p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    ...
                    <div x-show="selectedMeal" class="pt-2 border-t border-gray-200 dark:border-gray-700">
                        <form method="POST" action="{{ route('client.nutrition.store') }}">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <template x-for="type in mealTypes" :key="type">
                                        <button type="button" @click="mealType = type; customMealType = ''"
                                            :class="mealType === type && !customMealType ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600'"
                                ...
                                <input type="text" x-model="customMealType" @input="if(customMealType) mealType = ''" placeholder="Or type custom..."
                                    class="mt-2 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <template x-for="type in mealTypes" :key="type">
                                    <button type="button" @click="customFormMealType = type; customFormCustomType = ''"
                                        :class="customFormMealType === type && !customFormCustomType ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600'"
                                        class="px-3 py-1.5 text-sm font-medium border rounded-md transition-colors" x-text="type">
                                    </button>
                                </template>
                            </div>
