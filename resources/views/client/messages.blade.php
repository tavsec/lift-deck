    ...
    <div class="flex flex-col h-[calc(100vh-10rem)]">
        ...
        <div class="flex items-center pb-4 border-b border-gray-200">
            <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                <span class="text-lg font-medium text-blue-700">{{ strtoupper(substr($coach->name, 0, 1)) }}</span>
            </div>
            <div class="ml-3">
                <h1 class="text-lg font-medium text-gray-900">{{ $coach->name }}</h1>
                <p class="text-sm text-gray-500">Your Coach</p>
            </div>
        </div>
        ...
        <div id="messages-container" class="flex-1 overflow-y-auto py-4 space-y-4">
            ...
            @empty
                <div class="text-center py-12">
                    ...
                    <p class="mt-2 text-sm text-gray-500">No messages yet. Start a conversation with your coach!</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="pt-4 border-t border-gray-200">
            <form method="POST" action="{{ route('client.messages.store') }}" class="flex gap-2">
                @csrf
